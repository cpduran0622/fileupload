...
function ui_add_log_1(message, color)
{
  var d = new Date();
...
function ui_multi_add_file(id, file)
{
  ...
  template = template.replace('%%filesize%%', Math.round(file.size/1024));
...
function ui_multi_update_file_status(id, status, xhr)
{
 
  $('#uploaderFile' + id).find('span').html(xhr).prop('class', 'status text-' + status);
}
